...
void find_paths(set<int>& tiles, pair<int, short> place, unordered_map<pair<int, short>, pair<int, set<pair<int, short>>>, hash_pair>& places, set<pair<int, short>>& done) {
    tiles.insert(place.first);
    done.insert(place);
    //cout << places[place].second.size();
    for (const auto& prev : places[place].second) {
        //cout << prev.first << '\n';
        if (done.count(prev) || prev.first == -1) continue;
        find_paths(tiles, prev, places, done);
    }
}
...
int main() {
    ...
    unordered_map<pair<int, short>, pair<int, set<pair<int, short>>>, hash_pair> places;
    places[{pos, 0}] = {0, {{-1, -1}}};
    ...
    while (!next.empty()) {
        ...
        if (!done.count(totest) && (!places.count(totest) || node.first + 1 <= places[totest].first)) {
            set<pair<int, short>> temp = places[totest].second;
            temp.insert(node.second);
            places[totest] = {node.first + 1, temp};
            //cout << "hi" << '\n';
            ...
            if (!done.count(totest) && (!places.count(totest) || node.first + 1001 <= places[totest].first)) {
                set<pair<int, short>> temp = places[totest].second;
                temp.insert(node.second);
                places[totest] = {node.first + 1001, temp};
            ...
            if (!done.count(totest) && (!places.count(totest) || node.first + 2001 <= places[totest].first)) {
                set<pair<int, short>> temp = places[totest].second;
                temp.insert(node.second);
                places[totest] = {node.first + 2001, temp};
    ...
    set<int> bests;
    set<pair<int, short>> complete;
    find_paths(bests, {foundend.position(), -1}, places, complete);
    cout << bests.size() << '\n';
