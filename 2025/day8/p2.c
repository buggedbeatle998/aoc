...
void in_order(Node *tree, uint16_t *set_num, uint64_t *lens, uint16_t *next, const uint32_t NUM, uint32_t *pairs, uivec3 *boxes) {
    if (tree->left != NULL) {
        in_order(tree->left, set_num, lens, next, NUM, pairs, boxes);
    }
    ...
    if (*pairs == 0) {
        lens[0] = (uint64_t)boxes[tree->i1].x * (uint64_t)boxes[tree->i2].x;
        return;
    }
    ...
    if (tree->right != NULL) {
        in_order(tree->right, set_num, lens, next, NUM, pairs, boxes);
    }
}
...
int main(int argc, char **argv) {
    ...
    uint64_t *lens = calloc(NUM, sizeof(uint64_t));
    ...
    uint16_t next = 1;
    if (root != NULL) {
        in_order(root, set_num, lens, &next, NUM, &PAIRS, boxes);
        //for (uint16_t i = 0; i < NUM; ++i) {
        //    printf("%d\n", lens[i]);
        //}
    }
